...
def plot_phases_deepseek_comparison(phase2_file: str, phase3_file: str, phase4_file: str, output_path: str = None):
    """
    将DeepSeek在三个阶段的结果绘制在同一张图上
    ...
    for context_idx, context in enumerate(context_order):
        ...
        if context_idx == 0:
            # 添加透明图例占位符，不实际显示
            ax.plot([], [], 'o', color='#9ecae1', label='Phase 2', markersize=4)
            ax.plot([], [], '^', color='#4292c6', label='Phase 3', markersize=4)
            ax.plot([], [], 's', color='#08519c', label='Phase 4', markersize=4)
    
    # 添加整体标题
    fig.suptitle("DeepSeek在各阶段的偏见表现对比", fontsize=12, y=0.98)
...
def create_legend_chart(output_path: str):
    ...
    # 创建图例项
    ax.plot([], [], 'o', color='#9ecae1', label='Phase 2', markersize=6)
    ax.plot([], [], '^', color='#4292c6', label='Phase 3', markersize=6)
    ax.plot([], [], 's', color='#08519c', label='Phase 4', markersize=6)
...
if __name__ == "__main__":
    # 文件路径
    phase2_file = "/Users/lun/Downloads/work/研究生课程/危机决策与沟通/Data-for-Emergency 阶段一中文未改/outputs/phase2_deepseek.json"
    phase3_file = "/Users/lun/Downloads/work/研究生课程/危机决策与沟通/Data-for-Emergency 阶段一中文未改/outputs/phase3_deepseek.json"
    phase4_file = "/Users/lun/Downloads/work/研究生课程/危机决策与沟通/Data-for-Emergency 阶段一中文未改/outputs/phase4_deepseek.json"
    output_file = "/Users/lun/Downloads/work/研究生课程/危机决策与沟通/Data-for-Emergency 阶段一中文未改/outputs/phases_deepseek_comparison.png"
    
    # 生成对比图
    try:
        fig, axes = plot_phases_deepseek_comparison(phase2_file, phase3_file, phase4_file, output_file)
        print("DeepSeek多阶段对比图生成完成！")
